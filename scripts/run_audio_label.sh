...
for MODEL in "${LLM_MODELS[@]}"; do
    ...
    for ((RUN=1; RUN<=NUM_RUNS; RUN++)); do
        ...
        LOG_FILE="logs_eval/out_${MODEL}_${RUN}_trained.txt"
        ...
        python src/benchmark/RespLLM/RespLLM.py \
            ...
            --d_ff "$LLM_DIM" \
            --wandb_name "$run_name" \
            --save_pth cks/llm/out_${MODEL}_${RUN}_best_epoch.pt \
            2>&1 | tee -a $LOG_FILE
...
#         LOG_FILE="logs_5/out_${MODEL}_audiolabel_${RUN}.txt"
...
#             --use_audiolabel \
#             --wandb_name "$run_name" \
#             2>&1 | tee -a logs_5/out_${MODEL}_audiolabel_${RUN}.txt
