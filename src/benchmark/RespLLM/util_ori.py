...
# tạo metadata
    ...
    from_audio = False
# tạo spectrogram 
    spec_file_name = feature_dir + f"_spectrogram_pad{str(int(pad_len_htsat[dataset]))}" + suffix_dataset if dataset != "icbhidisease" else feature_dir + f"_segmented_spectrogram_pad{str(int(pad_len_htsat[dataset]))}" + suffix_dataset
...
# chia train/val/test cho từng dataset
    if dataset == "icbhidisease":
        x_data = np.load(feature_dir + f"_segmented_spectrogram_pad{str(int(pad_len_htsat[dataset]))}" + suffix_dataset)
        ...
        x_data = np.load(feature_dir + f"_spectrogram_pad{str(int(pad_len_htsat[dataset]))}" + suffix_dataset)
...
def test(model, test_loader, loss_func, n_cls, plot_feature="", plot_only=False, return_auc=False, verbose=True):
    ...
    with torch.no_grad():
        for i,  (x1, x2, x3, y) in enumerate(test_loader):
            ...
            _, predicted = torch.max(y_hat, 1) # lấy class có logits lớn nhất, dùng cho acc
            probabilities = F.softmax(y_hat, dim=1) # chuyển logits thành xác suất
