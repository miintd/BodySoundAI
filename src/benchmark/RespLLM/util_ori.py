...
def get_dataloader(configs, task, sample=False, deft_seed=None):
    ...
    if getattr(configs, 'ablate_medhistory', False):
        mask = np.array(["medical history" not in m for m in x_metadata_test])
        x_data_test, x_metadata_test, y_label_test = x_data_test[mask], x_metadata_test[mask], y_label_test[mask]
