# file chứa định nghĩa kiến trúc mô hình chính, bao gồm các thành phần cho từng phương thức (âm thanh, văn bản)
import numpy as np
import torch
...
# lớp phân loại cuối cùng, chuyển đầu ra thành vector 1 chiều để dự đoán
class FlattenHead(nn.Module):
    def __init__(self, nf, out_dim, head_dropout=0):
        super().__init__()
        self.flatten = nn.Flatten(start_dim=-2) # chiều bắt đầu flatten là chiều thứ 2 từ dưới lên
        self.linear = nn.Linear(nf, out_dim) # đây là lớp Linear cuối cùng; nf là kích thước của tensor sau khi flatten
...
# thử các model và kết hợp tất cả các thành phần lại với nhau
class RespLLM(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.head_dropout = configs.head_dropout # tỉ lệ dropout cho lớp Linear trong Flatten Head
    ...
    def forward1(self, x_spectrogram, x_prompt, x_context, no_fc=False):
        ...
        #print("x_spectrogram device:", x_spectrogram.device)
        #print("x_spectrogram dtype:", x_spectrogram.dtype)
        ...
        #print("enc_out device before:", enc_out.device)
        #print("enc_out dtype before:", enc_out.dtype)
        ...
        #print("prompt_embeddings device:", prompt_embeddings.device)
        #print("prompt_embeddings dtype:", prompt_embeddings.dtype)
        #print(prompt_embeddings.shape, enc_out.shape)
        enc_out = enc_out.to(device=target_device)  
        #print("enc_out device after:", enc_out.device)
        #print("enc_out dtype after:", enc_out.dtype)
        ...
        #print("dec_out device before:", dec_out.device)
        # print(dec_out.shape)
        ...
        # print(dec_out.shape)
        #print("dec_out device after:", dec_out.device)
        #print("dec_out dtype after:", dec_out.dtype)
    ...
    def forward(self, x_spectrogram, x_prompt, x_context, no_fc=False):
        ...
        dec_out = dec_out.permute(0, 2, 1).contiguous()
        # print(dec_out.shape)
        #print("dec_out dtype before", dec_out.dtype)
        dec_out = self.output_projection(dec_out[:, :, -self.patch_nums:], no_fc=no_fc)
        #print("dec_out dtype after", dec_out.dtype)
        # print(dec_out.shape)
        return dec_out
