...
from src.benchmark.MSP_ADF.model import MSP_ADF
# from src.benchmark.MSP_ADF.model_original import RespLLM_original

from src.benchmark.MSP_ADF.util import test, get_dataloader, EarlyStopper, set_all_seed

# from src.benchmark.MSP_ADF.util_ import get_dataloader_

from src.benchmark.MSP_ADF.sampler import CategoriesSampler
...
def train_MSP_ADF(configs):
    ...
    for epoch in tqdm(range(configs.train_epochs)):
        ...
        while True:
            ...
            try:
                ...
                x1, x2, x3, y, m = next(selected_iterator)
        ...
        if (epoch + 1) % configs.test_interval == 0:
            ...
            for j, test_loader in enumerate(train_loaders_):
                ...
                if n_cls[configs.train_tasks[j]] != configs.n_cls:
                    ...
                    test(model, test_loader, loss_func, configs.n_cls) #, plot_feature="llm/task" + configs.train_tasks[j] + "test")
    ...
    #     if (epoch + 1) == 3 or (epoch + 1) == 1 or (epoch + 1) == 2 or (epoch + 1) == 4 or (epoch + 1) == 5 or (epoch + 1) == 7 or (epoch + 1) == 8 or (epoch + 1) == 9 or (epoch + 1) == 10 or (epoch + 1) == 11:
    #         # configs.save_pth = f"cks/llm/model_{configs.llm_model}_{epoch+1}epoch.pt"
    #         configs.save_pth = f"cks/original/model_{configs.llm_model}_{epoch+1}epoch.pt"
    ...
    #         print(f'Save epoch {(epoch + 1)} to {configs.save_pth}')
...
def evaluate_MSP_ADF(configs):
    ...
    for task in configs.train_tasks:
        ...
        train_loaders.append(train_loader)
        val_loaders.append(val_loader)
        test_loaders_.append(test_loader_)
    ...
    # for j, train_loader in enumerate(train_loaders):
    #     print("Task", configs.train_tasks[j])
    #     print(f"Train loader {j} has {len(train_loader)} batches")
    #     for i in range(1): #range(configs.n_run_finetune):
    ...
    # for j, val_loader in enumerate(val_loaders):
    #     print("Task", configs.train_tasks[j])
    #     print(f"Train loader {j} has {len(val_loader)} batches")
    #     validation_loss += test(model, val_loader, loss_func, configs.n_cls) #, plot_feature="llm/task" + configs.train_tasks[j] + "val")
    
    # test on the seen tasks
    print("="*10 + "test on seen tasks")
    for j, test_loader in enumerate(test_loaders_):
        print("Task", configs.train_tasks[j])
    ...
    for j, test_loader in enumerate(test_loaders):
        ...
        if n_cls[configs.test_tasks[j]] != configs.n_cls:
            ...
            test(model, test_loader, loss_func, configs.n_cls, name= configs.test_tasks[j])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--patch_nums', type=int, default=1 , help='dimension of fcn') #64
    parser.add_argument('--d_ff', type=int, default=2304, help='dimension of fcn')  ###change default to 32 if want to use old weights
    parser.add_argument("--llm_dim",  type=int, default=2304)#768 for GPT2, 4096 for llama, openbiollm 2304
    ...
    parser.add_argument("--save_pth_pretrain", type=str, default=None)
    ...
    parser.add_argument("--attention", type=str, default="None") # flash, standard
    
    parser.add_argument("--spread_audio_embedding", type=str, default="none") # none, multi_view
    
    parser.add_argument("--model_type", type=str, default="None") 
    
    parser.add_argument("--classifier_head", type=str, default="flatten_head")
    
    parser.add_argument("--tweak", type=float, default=0)
