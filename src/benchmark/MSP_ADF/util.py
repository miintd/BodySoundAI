...
from src.benchmark.MSP_ADF.sampler import CategoriesSampler, SplitCategoriesSampler, TrainCategoriesSampler
...
class AudioDataset(torch.utils.data.Dataset):
    def __init__(self, data, from_npy=False, from_audio=False, prompt=""):
        ...
        self.label = data[2]
        self.modality = data[3] if len(data) > 3 else None
    ...
    def __getitem__(self, idx):
        ...
        # print("DEBUG: idx:", idx)
        # print("DEBUG: data dimensions:", len(self.data))
        # print("DEBUG: x shape:", x.shape)
        # print("DEBUG: label:", label)
        # print("DEBUG: metadata:", metadata)
        
        if self.from_audio:
            print("DEBUG: from audio")
            return x, label
        ...
        modality = self.modality[idx] if self.modality is not None else ""
        return x, self.prompt, metadata, label, modality
...
def get_dataloader(configs, task, sample=False, deft_seed=None):
    ...
    if dataset == "covid19sounds":
        ...
        print("DEBUG: y_label unique:", np.unique(y_label))
        print("DEBUG: y_label len:", len(y_label))
        print("DEBUG: x_data len:", len(x_data))
        print("DEBUG: Counter(y_label):", collections.Counter(y_label))
        ...
        if task in ["S3", "S4"]:
            ...
            # # ensure x_metadata is np.array
            # x_metadata = np.array(x_metadata)
            # # replicate to match y_set
            # if len(x_metadata) < len(y_set):
            #     repeat_factor = int(np.ceil(len(y_set) / len(x_metadata)))
            #     x_metadata = np.tile(x_metadata, repeat_factor)[:len(y_set)]
            # print(len(x_data), len(x_metadata), len(y_set))  # now both should match
...
def get_context(metadata):
    ...
    if "symptoms" in metadata:
        ...
        if pd.isna(userSymptoms) or userSymptoms == "" or "None" in userSymptoms:
            ...
            context += ", ".join([
                symptoms_dict.get(sym.strip(), sym.strip().lower())
                for sym in userSymptoms.split(",")
            ]) + ". "
...
def test(model, test_loader, loss_func, n_cls, plot_feature="", plot_only=False, return_auc=False, verbose=True):
    ...
    with torch.no_grad():
        for i,  (x1, x2, x3, y, m) in enumerate(test_loader):
            x1 = x1.to(device)
            # print(x3)
            y = y.to(device)
            # print(n_cls, y)
            ...
            if plot_feature: 
                feature = model(x1, x2, x3, m, no_fc=True)
